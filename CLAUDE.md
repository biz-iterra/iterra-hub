...
**作業の正本は `docs/tasks.md`（台帳）**、運用ルールは `docs/task-management.md`。

- **依頼・不具合報告を受けたら、着手より先に台帳へ起票する**（1 行でよい）
- **「完了」は本番で使える状態を指す。** 実装しただけなら「検証中」に留め、
  実施有無は「未」のまま。ローカルで動いたことを完了と呼ばない
- 「後で直す」「今回は記録のみ」と判断したものも起票する。判断自体が情報
- セッションの開始時に台帳の未完了を読み、着手先を決める

会話やコミットメッセージは管理台帳にならない。実際に
「エラー文言の日本語化」がマスタ管理の 4 箇所にしか入らないまま
セッションをまたいで放置された（2026-08-04）。
