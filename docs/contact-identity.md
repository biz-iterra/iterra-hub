...
dry-run のプレビューに区分を足す。

| 区分 | 意味 |
|---|---|
| 新規 | 既存に該当なし |
| 更新 | 同一人物・所属も同じ |
| **異動** | 同一人物・同じ会社で部署 or 役職が変わる |
| **転職** | 同一人物・会社が変わる |
| **統合候補** | 姓名一致のみ。別人として取り込むが要確認 |
| スキップ | エラー行 |

転職・統合候補は件数だけでなく**一覧を出す**（誰がどこからどこへ、という粒度）。
取り込む前に気付ける状態にすることが目的。
...
| Phase | 内容 | 完了の条件 |
|---|---|---|
| **A** | `contact_affiliations` 新設 / 既存データからの初期投入 / P2 の携帯判定追加 / 転職・異動の反映 / 連絡先詳細に所属履歴セクション | 転職後の名刺を取り込むと所属が切り替わり、旧所属が履歴に残る |
| **B** | `contact_merge_candidates` / P4 の候補記録 / dry-run の区分表示 / 統合候補の一覧画面 | 姓名一致のみの名刺が候補として画面に出る |
| **C** | `merge_contacts` / 統合 UI | 候補一覧から統合でき、関連データが引き継がれる |
