...
| 参照中の**唯一の**商談を削除しようとした | `この商談はリード「{リード名}」が参照している唯一の商談です。先にリードのステージを下げてから削除してください` |
| リード無しでセールスの商談を作ろうとした | `この商談には元になったリードが必要です。既存のリードを選ぶか、リードを新規作成してください` |
| TQL 未満のリードで商談を作ろうとした | `リード「{リード名}」は「{ステージ名}」段階です。商談を作るには「選定」以上へ進めてください` |
| 削除済みのリードに商談を紐づけようとした | `削除されたリードには商談を紐づけられません` |
...
**契約を商談へ紐づけるときの取り違え**（`src/actions/contracts.ts`）。候補一覧や
編集画面を開いたまま放置している間に、他の人が同じ契約を触ったときに出る。

| 事象 | メッセージ |
|---|---|
| すでにこの商談に紐づいている | `この契約はすでにこの商談に紐づいています` |
| すでに別の商談に紐づいている | `この契約はすでに別の商談に紐づいています。もとの商談で紐づけを解除してから操作してください` |
| すでにこの商談から外れている | `この契約はすでにこの商談から外れています。画面を再読み込みしてください` |
