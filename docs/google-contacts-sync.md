...
- スコープ: `https://www.googleapis.com/auth/contacts` のみ（§2.1）。
  `granted_scope` を保存しスコープ逸脱を監査する（Gmail と同じ）
- **GCP プロジェクトは Gmail 連携と同じものを使い、OAuth クライアントだけ分ける**
  （2026-08-05 に訂正。当初は専用プロジェクトを作る案だった）
  - 同意画面の「内部 / 外部」は**プロジェクト単位**だが、**Gmail 連携も既に「内部」**
    （Workspace アカウント限定。`database-design.md` §20.4）。同じ「内部」どうしなので
    同居でき、プロジェクトを分ける理由にはならない
  - 内部アプリなら Google の審査が不要で、**Workspace 組織のアカウントしか認可できない**
    （会社アカウント限定を Google 側でも強制できる）。これは両連携に等しく効く
  - **クライアントを分けるのは `include_granted_scopes` があるため。** Gmail の
    認可 URL は `include_granted_scopes: "true"` で過去のスコープを引き継ぐ
    （`src/lib/gmail/client.ts`）。クライアントを共用すると Gmail の再連携時に
    contacts の権限まで引きずったトークンが出て、「1 接続 = 1 目的のスコープ」を
    前提にした `granted_scope` の監査が意味を失う
...
### 2.1 スコープは 1 段階しかない（2026-08-05 に認識を訂正）

**書き込めるスコープは `auth/contacts` の 1 つだけ**で、同意画面には次のように出る。

> 連絡先の表示、編集、ダウンロード、完全な削除

「読み書きのみ」といった控えめな権限ではない。**技術的には利用者の全連絡先を
消せる権限**を受け取る。People API には「書き込みだけ」「特定のグループだけ」という
絞り込みが用意されておらず、選択の余地がない。

| スコープ | できること | 本連携で使えるか |
|---|---|---|
| `auth/contacts` | 表示・編集・ダウンロード・**完全な削除** | **これしか無い**（push に必要） |
| `auth/contacts.readonly` | 表示のみ | 使えない（書けない） |
| `auth/contacts.other.readonly` | 自動収集の「その他の連絡先」の表示のみ | 対象外 |

#### 与えられた権限をどう抑えるか

Google 側では制限できないので、**アプリ側の実装で範囲を絞る**。
これは Google が保証する制限ではなく、こちらの規律である点を理解しておくこと。

| 抑制 | 実装 |
|---|---|
| 触る対象を限る | 同期対象は「ITERRA CRM」グループの中だけ（§3）。個人の連絡先は読み書きしない |
| 消す対象を限る | 削除するのは**リンク表にあり、かつ CRM 側で論理削除された連絡先だけ**。「Google 側にあって CRM に無いもの」を掃除する処理は作らない |
| 消えない項目を残す | 更新は `updatePersonFields` に挙げた項目のみ。写真・メモ等は触らない（§4.6） |
| 権限の逸脱を見る | `granted_scope` を保存し、`auth/contacts` 以外の contacts 系が付いていたら接続を中止する |

#### 利用者への説明

**同意画面に「完全な削除」と出ることを事前に伝える。** 何も知らせずに出すと、
不信感を招くか、連携そのものを止められる。`/profile` の連携欄に、
何を触り何を触らないかを明記している。

### 2.2 環境変数（シークレット管理の規約に従い Bitwarden → 転記）
...
1. **Gmail 連携と同じ GCP プロジェクト**を使う（新しく作らない。§2）
2. People API を有効化する
3. OAuth 同意画面が**「内部」**であることを確認する
   （Gmail 連携のために既にそうなっているはず。`database-design.md` §20.4）
4. **OAuth クライアント（ウェブ）を新しく作る**（Gmail のものを使い回さない。§2）。
   コールバック URI を登録する
...
| 専用の GCP プロジェクトを作る | **当初はこの案だった**が、「Gmail 連携が外部アプリだから同居できない」という前提が誤りだった（Gmail も §20.4 で「内部」と決定済み）。同じ「内部」どうしなら同居でき、プロジェクトを増やす分だけ管理と設定漏れの余地が増える。**分けるべきなのは OAuth クライアントだけ**（2026-08-05 に訂正） |
