...
| 段 | 条件 | 挙動 |
|---|---|---|
| L1 | `source_external_key` が既存 Lead と一致 | **空欄のみ補完**。既存値は上書きしない |
| L2 | `contact_email` が既存 Lead と一致（キー未設定） | L1 と同様に扱い、キーを紐付ける |
| L3 | 正規化会社名 + 姓 + 名 が一致 | 同一人物の可能性 → **既定はスキップ**。プレビューで一覧表示 |
| L4 | ドメイン一致のみ | 同じ会社の別人 → 新規作成（実測 144 社が複数名刺・最大 8 枚） |
