...
**例外は所属（会社・部署・役職）で、名刺交換日が現在の所属の開始日より新しければ更新する**
（`apply_contact_affiliation`）。旧所属は `contact_affiliations` に履歴として残る。
...
連絡先側では、この 24 件は所属の切り替えとして `contact_affiliations` に旧所属・新所属の 2 行で残る。
