...
- 理由: 商談の相手は「Ａ社のＢさん」であることが普通で、事業者情報と連絡先は
  同時に紐づく（T-0064）。**1 件だけ返す版を詳細ページで使うと連絡先が
  事業者情報の陰に隠れる**（実際に隠れていたのを実操作で発見した）

### UT-74: 契約のバリデーション（商談は任意・金額・楽観ロック）
...
- 理由: 紐づけ・解除は候補一覧や編集画面を開いたまま放置されうる。楽観ロックを
  ...
  解除で `deal_id` を突き合わせるのは、古い画面から別の商談の紐づけを外さないため

### UT-75: 商談を作れるリードかの判定
- 対象: `src/lib/deals/lead-requirement.ts`
- 期待値: 選定 / Sales / Opportunity は作れる。獲得・育成・**Dead** は作れず
  「ステージを上げれば作れる」と伝える。リード未選択は上げても解決しないと伝える。
  **ステージが取れないときは止めない**（RLS で埋まらないことがあり、
  そこで止めると理由の分からない行き止まりになる。DB 側で弾く）
- `pickRaiseTargetStage`: 上げ先は「商談を起こしてよく、まだ商談を前提としない段階」（選定）。
  **Sales 以降は選ばない**（商談が無いうちは上げられない）
- 理由: 商談の新規作成画面と Server Action で同じ判定を二重に書かないため

### UT-76: 商談のバリデーション（取引先を受け取らない）
