...
- 理由: 契約前のディール（account_id = NULL）は日常状態。フォールバック 2 段目の検証
...
- 入力: `"[lead_name] リード名は必須です"` / `"診断マスタが未投入です"` / `conflictErrorMessage("ディール")` の戻り値 / `null` / `""`
...
### 4.16 `src/lib/freee/prefecture.ts` — 都道府県コードの変換（実装済み: `prefecture.test.ts`）

### UT-63: freee の都道府県コードと和名の往復
- 入力: 0 / 46 / -1 / 47 / null / undefined、和名（前後に空白があるもの含む）
- 期待値: **0 が北海道、46 が沖縄県**（0 始まり）。範囲外・未設定は null。
  判定できない名前（「東京」など）は null で、**送らない**
- 理由: 1 始まりと取り違えると全県が 1 つずれる。DB 側にも同じ対応表があり
  （`freee_prefecture_name` / `freee_prefecture_code`）、**片方だけ直すと
  取り込みと送信で県が食い違う**

### 4.17 `src/lib/freee/payload.ts` — freee へ送る本体の組み立て（実装済み: `payload.test.ts`）

### UT-64: buildFreeeUpdatePayload — 送ってよい項目だけを組み立てる
- 入力: `code` 単独 / `code` と `phone` の同時指定 / `name`（値あり・null・空文字）/
  住所 4 項目 / 口座 5 項目 / `qualified_invoice_issuer`・`org_code` の表示語 / 未知の項目名
- 期待値: **`code` は常に落とす**。同時に選ばれた他の項目は残る。`name` が空なら
  `name` も `long_name` も入れない。住所は 1 つの `address_attributes`、口座は 1 つの
  `partner_bank_account_attributes` にまとまる。「該当する」→ `true`、「個人」→ `2`、
  当座は `current` → `checking`。未知の項目名は無視する
- 理由: freee の**更新** API に `code` は無く、混ぜると 400
  「このAPIでは code の指定はできません。」で**その回の全項目が落ちる**（2026-08-05 の報告）。
  元は Server Action の中に `Record<string, unknown>` で書かれていて型もテストも
  効かなかったため、純粋関数へ切り出した。`name` を空で送ると同様に更新全体が落ちる

### UT-65: buildFreeeCreatePayload — 新規登録で送る本体
- 入力: 最小構成（コードと名称だけ）/ 空白のみの値 / インボイス番号あり・なし ×
  適格フラグ / 住所 4 項目 / 判定できない都道府県（「東京」）/ 口座（`current`）/
  `org_code` が 1・2・null
- 期待値: **取引先コードが必ず入る**。値の無い項目・空白だけの項目は**送らない**。
  インボイス番号が無いのに適格フラグだけ送らない。都道府県が判定できなければ
  住所ごと送らない。当座は `current` → `checking`。`org_code` は 1 / 2 のみ通す
- 理由: 事業所設定が「取引先コードを使用する」のため、コードが空だと
  400「Codeを入力してください。」で登録できない。**取引先コードを入れられるのは
  登録のときだけ**で、失敗しても後から入れ直せない（§26.8）。
  空文字を送ると freee 側に空欄を作る
- 併せて確認するもの（2026-08-05 追加）: **会社名が `name`（基本情報の名前）と
  `long_name`（書類の正式名称）の両方に入る**こと、カナが `name_kana`
  （正式名称（カナ））に入ること、**敬称に既定の「様」が入る**こと。
  既定値は DB の `freee_default_title()` と対で持つので、変えるときは両方（§26.8.2）

### 4.18 `src/lib/search-field.ts` — 検索欄の判断（実装済み: `search-field.test.ts`）

### UT-66: 日本語入力（IME）の変換中は検索を走らせない
- 入力: `compositionStart` → `input`（未確定）× n → `compositionEnd` の並び／
  `input` が `compositionEnd` より先に来る並び（ブラウザ差）／変換を伴わない入力／
  変換を連続 2 回／変換を取り消して空で確定／`shouldIgnoreKey` の 3 通り
- 期待値: **変換中の未確定値は親へ伝えない**。確定値だけが伝わる。表示（draft）は
  変換中も追従する。`compositionEnd` は前後どちらの並びでも確定値を伝える。
  変換を確定させた Enter（`isComposing`）と変換中のキーは無視する
- 理由: 未確定の文字で検索すると結果の再描画が変換中の入力を壊し、
  「打った文字が消える・確定できない」になる。**同じ指摘を何度も受けている**ため、
  判断を画面から引き剥がして純粋関数にし、ここで仕様を固定した（2026-08-05）。
  画面への配線は E2E-14 が見る

### 4.19 `src/lib/google-contacts/` — Google コンタクト同期（実装済み: `mapping.test.ts` / `sync.test.ts`）

### UT-67: mapping — CRM と Google の項目対応
- 入力: 姓名・カナ / メール・電話・住所（ラベル各種）/ 会社・部署・役職 /
  誕生日（年あり・年なし・形式違い）/ 空文字・空白のみ / 未知の Google type /
  削除済みの連絡先
- 期待値: 姓名は**構造のまま**渡す。ラベルは §4 の表どおり変換し、**知らない type は
  other へ寄せる**。主のものを先頭に置く。会社・部署・役職が全て無ければ
  `organizations` を送らない。**年なしの誕生日は取り込めない**（CRM は DATE）ので
  `birth_date` は null にし、表示用に別で持つ。空文字は送らない
- 理由: 対応表の正本は `docs/google-contacts-sync.md` §4。**ここが仕様の固定点**で、
  表を変えたらテストも変える。freee で Server Action に組み立てを書いて型もテストも
  効かなかった失敗を繰り返さないため、純粋関数に切り出してある

### UT-68: mapping — 指紋（変更検出）
- 入力: 同じ内容で並び順が違うもの / メールの大文字小文字違い / 電話の書式違い /
  同期対象の項目を変えたもの
- 期待値: **並び順・書式が違っても内容が同じなら同じ指紋**。同期対象が変われば変わる
- 理由: `contact_emails` / `contact_phones` に `updated_at` が無く、親の `updated_at` も
  子の変更で動く保証が無いため、内容そのもので変更を検出する（§5.2）。
  正規化が甘いと毎回 push してクォータを浪費する

### UT-69: sync — push の分岐
- 入力: 未リンク / 指紋が同じ / 指紋が違う / **etag が無い** / CRM で論理削除済み /
  1 件目が失敗する 2 件
- 期待値: 未リンクは作成して**必ずグループへ入れる**。指紋が同じなら送らない。
  違えば更新する。**etag が無ければ更新しない**（黙って上書きしない）。
  論理削除済みは Google からも消す。**1 件の失敗で同期全体が止まらず**、失敗も記録する
- 理由: グループが同期対象の境界（§3）なので、入れ忘れると次回から対象外になる。
  etag はドキュメントどおり必須で、無視すると Google 側の編集を消す

### 4.20 `src/lib/change-log-format.ts` — 変更履歴の日本語化（実装済み: `change-log-format.test.ts`）

### UT-70: 変更履歴を人が読める形にする
- 入力: 共通列 / テーブル固有の列 / マスタの制御列 / 対応の無い列 /
  null・空文字・真偽値・ISO 日時・日付・UUID・長い文字列・配列・オブジェクト /
  更新の差分 / 作成（`_row`）/ 削除（`deleted_at` のみ）/ `_name` 付き
- 期待値: 列名は日本語へ（**対応が無ければ列名のまま。隠さない**）。
  UUID は名前へ、引けなければ「他のデータ」（**生の UUID を出さない**）。
  日時は和式で秒とタイムゾーンを落とす。null は「未設定」、真偽値は「はい/いいえ」。
  **作成は全カラムの JSON を並べず対象名だけ**。削除は `deleted_at` を主役にせず
  「何を消したか」を出す
- 理由: 「システムログをそのまま表示させているだけ」という指摘（2026-08-05）。
  実際に `deleted_at: 空 → 2026-08-05T10:28:37.027+00:00` や
  `_row: {"id":"51e0646e-…"（数百文字）` が画面に出ていた

### UT-71: Zod の既定エラー文言を日本語にする
- 入力: `invalid_type`（未入力 / 型違い）/ `too_small` / `too_big`
  （string・number・array・date、境界を含む場合と含まない場合）/
  `invalid_format`（email・url・uuid・未知の形式）/ `invalid_value` / `custom`
- 期待値: どの種類でも日本語を返す。**`1 文字以上` と未入力は「入力してください」に寄せる**
  （文字数を言われても直しようがない）。`custom` は `undefined` を返し、
  refine 側の文言に委ねる
- あわせて **`z.config` が実際に効いているか**をスキーマの `safeParse` で確かめる。
  判定関数が正しくても配線されていなければ英語のまま出るため、
  結果の文言が ASCII だけで構成されていないことを検査する
- 理由: `docs/error-messages.md` §1 の「英語の生エラーを出さない」は
  **個別のスキーマに書き忘れると破れる**。2026-08-04 の全面適用は DB エラーだけで、
  Zod は手つかずだった（T-0018）。**Zod を上げたときは最初にこのテストを走らせる**
  （issue の形が変わると静かに英語へ戻る）

### UT-72: 口座種別の比較の正規化
- 入力: null / undefined / 空文字 / 空白のみ / 値あり（前後に空白）
- 期待値: 未設定はすべて `ordinary`。値があれば前後の空白だけ落とす
- あわせて**揃えても本当の差分が消えないこと**を見る
  （freee=当座 × CRM=未設定 は残る、freee=普通 × CRM=貯蓄 も残る、
  freee=普通 × CRM=未設定 だけ消える）
- 理由: **freee は口座種別に未設定を持てない**（未選択でも `ordinary` が返る）。
  CRM は NULL を取れるので、素で比べるとどちらも未設定なのに差分として並ぶ
  （2026-08-06 の指摘）。DB 側の `normalize_account_type` と対の規則で、
  **片方だけ直すと差分の出方が食い違う**

### UT-73: ディールの相手先表示（1 件だけ返す版と全件返す版）
- 対象: `src/lib/deal-counterparty.ts`
- 入力: 取引先 / 事業者情報 / 連絡先の有無の組み合わせ
- 期待値:
  - `getDealCounterparty` は**優先順位で 1 件**（取引先 → 事業者情報 → 連絡先）。
    一覧・カンバンのように 1 行で示す場所向け
  - `getDealCounterparties` は**紐づくものを全部**同じ順で返す。詳細ページ向け
  - 名が欠けた連絡先は姓だけで組み立てる（名刺由来のデータで起きる）
- 理由: ディールの相手は「Ａ社のＢさん」であることが普通で、事業者情報と連絡先は
  同時に紐づく（T-0064）。**1 件だけ返す版を詳細ページで使うと連絡先が
  事業者情報の陰に隠れる**（実際に隠れていたのを実操作で発見した）

### UT-74: 契約のバリデーション（ディールは任意・金額・楽観ロック）
- 対象: `src/lib/validators/contracts.ts`
- 期待値:
  - `deal_id` を渡さなくても・`null` を渡しても通る（**未紐づけの契約を作れる**。T-0065）
  - `amount` は 0 以上の整数のみ。負数と小数は落とす
  - `linkContractToDealSchema` / `unlinkContractFromDealSchema` は
    **`expected_updated_at` が無いと落ちる**
  - 解除でも `deal_id` は必須
- 理由: 紐づけ・解除は候補一覧や編集画面を開いたまま放置されうる。楽観ロックを
  任意にすると、その間に他の人が触った紐づけを後勝ちで壊す。
  解除で `deal_id` を突き合わせるのは、古い画面から別のディールの紐づけを外さないため

### UT-75: ディールを作れるリードかの判定
- 対象: `src/lib/deals/lead-requirement.ts`
- 期待値: リード選定 / ディール / オポチュニティ は作れる。リード獲得・ナーチャリング・**デッド** は作れず
  「ステージを上げれば作れる」と伝える。リード未選択は上げても解決しないと伝える。
  **ステージが取れないときは止めない**（RLS で埋まらないことがあり、
  そこで止めると理由の分からない行き止まりになる。DB 側で弾く）
- `pickRaiseTargetStage`: 上げ先は「ディールを起こしてよく、まだディールを前提としない段階」（選定）。
  **ディール 以降は選ばない**（ディールが無いうちは上げられない）
- 理由: ディールの新規作成画面と Server Action で同じ判定を二重に書かないため

### UT-76: ディールのバリデーション（取引先を受け取らない）
- 対象: `src/lib/validators/deals.ts`
- 期待値: 新規作成は相手先が 1 つ以上必要。**`account_id` を渡しても相手先と見なさない**
  （取引先は契約成立時に自動で作られるもので、作る時点では存在しない）。
  `createDealWithLeadSchema` は mode ごとに必須が変わる
- 理由: 2026-08-07 に DB 制約へ合わせて取引先を選べるようにしたが、
  業務の流れとして筋が通っていなかった（利用者の指摘。T-0070）
