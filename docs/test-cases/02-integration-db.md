...
## 住所の更新は 1 トランザクション（2026-08-14 追加、T-0104 / T-0096）

マイグレーション `20260814100006` の検証。自動化は
`scripts/test-db/cases/14-update-entity-address.mjs`。

住所は本体（`addresses`）と紐付け（`entity_addresses`）の 2 表に分かれている。
追加（`add_entity_address`）は元から DB 関数だったのに、**更新だけアプリが
2 文に分けて UPDATE** していた。片方だけ通ると「住所は変わったのにラベル・電話が
古いまま」という食い違いが残る。楽観ロックも同じ関数で見る。

### IT-ADDRUPD-01: 本体と紐付けが同時に変わる
- 手順: `update_entity_address` でラベル・電話・都道府県・市区町村・郵便番号をまとめて変える
- 期待値: 5 項目すべてが新しい値になる
- 理由: 2 文に分けていた頃は、片方だけ通る余地があった

### IT-ADDRUPD-02: 空文字は NULL に落とす
- 手順: 番地に空白のみ、電話に空文字を渡す
- 期待値: どちらも NULL で保存される
- 理由: 追加側（`add_entity_address`）と規則をそろえる。片方だけ空文字が残ると
  「空欄なのに検索に引っかからない」といった食い違いになる

### IT-ADDRUPD-03: 楽観ロックの競合で巻き戻る
- 手順: 古い `updated_at` を渡して更新する
- 期待値: `CONFLICT:` を含む例外。**紐付けも住所本体も元のまま**
- 理由: 紐付けを先に更新して判定するので、競合時に住所本体まで進まない

### IT-ADDRUPD-04: 持ち主が違えば更新できない
- 手順: 別の連絡先の ID で他人の紐付け ID を指定する
- 期待値: 「住所が見つかりません」で例外
- 理由: RLS だけに頼らず、関数側でも持ち主ごと特定する
