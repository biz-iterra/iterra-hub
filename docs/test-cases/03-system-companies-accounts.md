...
| `/accounts/[id]` | 取引先詳細（ディール一覧・窓口の連絡先・区分バッジ） |
...
- Deal（ディール）・Contract（契約）自体の CRUD は別領域。本書では Account 自動作成の受け側検証（ACC-15）のみ扱う

## 2. テストケース

### COM-40: freee 連携の状態がアイコンで分かる（2026-08-04 追加）

- 対象: 事業者情報の一覧・詳細（`FreeeLinkIcon`）
- 権限: admin / member
- 手順:
  1. admin で一覧を開き、freee と紐づいている事業者と紐づいていない事業者を見比べる
  2. 詳細を開く
  3. member で同じ一覧を開く
- 期待結果:
  - 1: **紐づいていればカラー、いなければグレー**。マウスを乗せると状態が読める
    （「freee と連携済み（確定）」「freee と未連携」など）
  - `excluded`（対象外と判断したもの）は**連携済みの色にしない**
  - 2: 事業者名の横に同じアイコンが出る
  - 3: **member にはアイコンが出ない**（`freee_partners` は admin しか読めず、
    出すと連携済みでも未連携に見えるため）
- 自動化区分: 自動(Playwright)（E2E-13 が未連携の表示を担保）
...
- 権限: manager
- 事前条件: 事業者情報・ディール・窓口連絡先・区分が紐づく取引先が存在する
- 手順
  1. 対象取引先の詳細ページを開く
- 期待結果
  - ヘッダー: account_code + 取引先名 + 「編集」ボタン
  - 左（メイン側）: 基本情報（取引先名・担当者・事業者情報・説明）/ 属性情報（種別・ステータス・リードソース・インボイス登録・登録番号・ステータス更新日・区分バッジ）/ **ディール一覧テーブル**（コード・取引名・ステージ・ステータス・金額。金額は ¥ 表記）
  - インボイス登録番号は**紐づく事業者情報の値の読み取り表示**（取引先側では編集できない。編集ページにも欄が無い）
  - 右サイドバー: 「窓口の連絡先」は**リスト型**（テーブルはメイン側のディール一覧のみ）。各行に氏名・部署/役職・区分ラベル
  - 削除済み連絡先は窓口一覧に出ない
...
### ACC-12: 論理削除（編集ページ内モーダル・アクティブディールのブロック)

- 対象: deleteAccount
- 権限: admin
- 手順
  1. アクティブなディール（closed_at IS NULL かつ未削除）が紐づく取引先の編集ページで「削除」→ モーダルで「削除する」
  2. ディールのない（またはすべてクローズ済みの）取引先で同じ操作を行う
- 期待結果
  - 手順 1: エラー「アクティブなディールが存在するため削除できません」。削除されない
  - 手順 2: 成功トースト「取引先を削除しました」、`/accounts` へ遷移。DB は deleted_at / deleted_by セットの論理削除（SQL 検証）
  - クローズ済みディールだけが紐づく取引先は削除できる（closed_at 条件の確認）
- 自動化: Playwright候補 + SQL検証
...
- 事前条件: account_id が NULL で company_id / contact_id を持つディール（Lead 昇格由来）が存在する
- 手順
  1. そのディールに対して契約を登録する（契約画面の操作手順は契約領域の仕様書に従う）
  2. `/accounts` 一覧を開く
  3. 自動作成された取引先の詳細を開く
- 期待結果
  - Account が自動作成され、ディールの account_id に紐づく（SQL 検証: deals.account_id が非 NULL になる）
  - 詳細ページで、ディールのパイプラインに対応する区分が assigned_by_contract = true で付与されている（編集ページの区分行に「契約により自動付与」と表示）
  - ディールの相手担当者が「窓口の連絡先」に主担当（primary）として入っている
