...
### CNT-42: 従属テーブルの更新も後勝ちにしない（T-0096）

- 対象: 連絡手段（メール・電話）の種別変更、住所の編集、SNS・チャットの編集、
  タレントの経歴編集。`updateContactChannelLabel` / `updateEntityAddress` /
  `updateContactSocialAccount` / `updateTalentCareer`
- 権限: 担当者本人 または manager 以上
- 手順:
  1. 同じ連絡先の編集ページを 2 つのタブで開く
  2. 片方でメールの種別を変えて保存する
  3. もう片方でも同じ行の種別を変えて保存する
- 期待結果:
  - 2 回目が「…は他のユーザーによって更新されています。画面を再読み込みしてから
    保存してください」で拒否される
- 補足:
  - **これらは親フォームとは別の Server Action を直接呼ぶ**ので、連絡先本体の
    楽観ロックでは守られない。行ごとに `updated_at` を渡す
  - `contact_emails` / `contact_phones` は `updated_at` 列そのものが無く、
    `20260814100004` で追加した。既存行は `created_at` に揃えてある
- 自動化区分: 手動
