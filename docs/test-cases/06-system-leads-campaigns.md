...
### LD-28: 同じ保存で入力した値が昇格に届く（2026-08-04 追加）

- 対象: `updateLead` の保存順序 / `promoteLeadToDeal` の必須チェック
- 権限: admin
- 背景: 昇格は**DB の値を読む**。2026-08-04 に昇格を leads の更新より先へ動かした結果、
  「今回の編集で入力した値」が届かず、**seed 由来のリード（事業者種別が未設定）の
