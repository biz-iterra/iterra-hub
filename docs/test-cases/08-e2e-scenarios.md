...
4. 他人の company 詳細へ直 URL → 「見つかりません」
...
### 6.1 既知の問題（テスト側で回避。アプリ側は未修正）

- **`router.push(id); router.refresh();` の直後連続呼び出しでクライアント遷移が固まることがある**
  （`e2e/helpers.ts` 冒頭に詳細コメントあり）。lead-new-form / company-new-form /
  contract-new-form や各 edit-form の保存・削除後の遷移に広く同じパターンが使われている。
  Playwright（headless Chromium + Turbopack dev）から高速に操作すると、保存トーストは出るのに
  URL が古いページのまま変わらないことがある（DB への保存自体は成功している）。
  E2E 側は「その操作自身の自動遷移を `waitForURL` で待たない」方針で回避した
  （検索結果のリンクや既知の ID への `page.goto()` で次に進む）。
  再現条件・影響範囲の精査と修正は別課題とする。
