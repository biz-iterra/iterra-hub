...
## リリース: 2026-08-14 スクロール崩れの修正 + 全体監査 + 残課題の解消（マイグレーション 6 本）

- 対象コミット: `c66c754`（T-0089〜T-0093）→ `cde7f5b`（監査の起票）→ `e95a759`（T-0094）→
  `fbef566`（T-0095 / T-0097 / T-0098 / T-0101）→ `4549948`・`b290823`（T-0096）→
  `35d337e`（T-0102）→ `46769f6`（T-0103 / T-0104）
- 変更領域: `AppShell` の `<main>`、`src/lib/app-scroll.ts` / `src/hooks/useScrollLock.ts`（新設）、
  モーダル 7 つ、`src/actions` 10 ファイル、マイグレーション 6 本
  （`20260814100001`〜`100006`）
- 対応タスク: T-0089 〜 T-0104（13 件を修正、2 件は規約側・テスト側の是正）
- 回帰範囲の判定: 画面の外枠と Server Action・RLS の広範囲に触れるため、**E2E 全件 + 総ざらい +
  DB テスト全件**を実施
...
| Gate 5 デプロイ後スモーク | 2026-08-15 | Claude | 合格 | マイグレーション 6 本の適用を `migration list` で確認。NAS の更新後に本番の画面で確認（下記） |

### Gate 5 の結果（2026-08-15、NAS 更新後）

**発端の症状が本番で解消していることを実測で確認した。**

| 見たこと | 結果 |
|---|---|
| `<main>` の実際の指定 | `position: relative` / `id="app-scroller"`（新しいイメージが動いている証拠） |
| `/activities` の文書スクロール | **782px → 0px** |
| `/dashboard` の文書スクロール | **271px → 0px** |
| `/leads` の文書スクロール | 0px |
| 本文を最後まで送ったあと | `window.scrollY` は 0 のまま、ヘッダーとサイドバーが固定 |
| 主要 12 画面の応答 | すべて 200（`/dashboard` `/leads` `/sales` `/contacts` `/companies` `/accounts` `/contracts` `/projects` `/talents` `/campaigns` `/admin` `/contacts/cards`） |
| マスタ管理のモーダル | 開いている間 `main` が `overflow: hidden` で背面が動かない。閉じると `auto` に戻る |
| モーダルの収まり | 画面高 855px に対し上端 119px・下端 737px |
| ディール詳細 | 表示・活動とも正常。RLS 変更後もエラーなし |
| コンソール | 再読み込み後もエラー・RLS 拒否（42501）なし |

**書き込みを伴う確認（リードの昇格・ディールの新規作成で履歴が 1 組だけ入ること）は
本番データを増やすため実施していない。** 通常業務で 1 件目が作られたときに
`deal_stage_histories` の件数を見れば足りる（DB としては IT-DEALHIST-05 で固定済み）。

### 検出した不具合と処置

- **T-0089（本番で報告された症状の原因）**: `<main>` が非配置のため `.sr-only`
  （`position: absolute`）の包含ブロックが `<html>` になり、`overflow-y: auto` に
  クリップされずに文書のスクロール領域を押し広げていた。本番の `/activities` で 782px、
  `/dashboard` で 271px を実測。`position: relative` を足して 0px に。E2E-19 を新設し、
  **`relative` を外すと落ちること**も確認した
- **T-0100（Gate 3 で検出）**: 総ざらいの「個人事業主の作成」が T-0087 に追随しておらず、
  必須になった姓名を埋めずに保存してブラウザの標準検証で止まっていた。総ざらい側を修正
- **E2E-19 の入力エラーケースが seed 依存だった**: 一覧 1 行目の連絡先の必須項目が
  空だと標準検証で止まる。保存前に空の必須欄を埋める形にした
- **E2E の失敗はすべて環境要因だった。** 内訳は次のとおり
  - `.next` を作り直した直後の回: E2E-04 / E2E-07 / E2E-17 と総ざらいが
    「Test timeout of 90000ms exceeded」「waitForURL: Timeout」。各ページの初回
    コンパイルが上限を超えていた。温まった状態で再実行して 4 件とも緑
  - **E2E-04 / E2E-17 には `test.setTimeout(240s)` を入れた。** 画面をまたぐ手順が
    多く、後片付けまで含めると既定の 90 秒に収まらない。切れると後片付けの
    途中で止まってデータが残る
  - 最終確認の 1 回目で E2E-16 が 1 件だけ落ちた（事業者作成のトーストが 10 秒
    以内に出ない）。**単独・直前 3 本との連続・総ざらいのいずれでも再現せず、
    もう一度全件を回して 36/36 緑**。単発の揺らぎと判断した。
    **アーティファクトは次の実行で上書きされてしまい、失敗時の画面は追えていない**
  - `npm run build` は dev サーバーと同じ `.next` を書き換える。**E2E の直前に
    build を回したらサーバーを作り直すこと**（この取り違えで 1 回無駄にした）

### テストケースへの追記

- E2E-19（`e2e/19-page-scroll-layout.e2e.ts`、5 ケース）: スクロールで外枠がずれない /
  背面のスクロールロック / 低い画面でのモーダル / 貼り付いた右カラム / 入力エラーで先頭へ戻る
- E2E-20（`e2e/20-lead-sub-owners.e2e.ts`）: 副担当付きリードの作成が 1 トランザクション
- IT-DEALHIST-01〜06（`scripts/test-db/cases/13-deal-history-trigger.mjs`）: ディールの履歴トリガー
  （作成時の初回履歴が 1 組だけ・直接 INSERT できないことを含む）
- IT-ADDRUPD-01〜04（`scripts/test-db/cases/14-update-entity-address.mjs`）: 住所更新の
  トランザクションと楽観ロック
- CMN-13 / CMN-14 / CMN-15（07）、ADM-03b（07）、CNT-42（04）

### 申し送り

- **本番反映にはマイグレーション 6 本の適用が必要**（`20260814100001`〜`100006`）。
  `20260814100003` / `100005` は RLS の変更を含むので、反映後に
  「ディールの活動が担当者に見える」「見られないディールの活動が見えない」を確認すること
- `20260814100005` は `promote_lead_to_deal` / `create_deal_with_lead` を差し替える。
  **反映後にリードの昇格とディールの新規作成を 1 件ずつ通し、履歴が 1 組だけ入ることを見る**
- `20260814100004` は `contact_emails` / `contact_phones` に列を足す。既存行は
  `created_at` に揃うので、**「たった今更新された」ように見える行は出ない**
