...
| Gate 4 受入 (09) | 2026-08-03 | Claude + プロダクトオーナー | **不合格 → 修正後に再確認待ち** | 実操作で 4 件の指摘。うち 3 件は不具合として修正、1 件は仕様変更。下記参照 |
...
| 報告 | 判定 | 処置 |
|---|---|---|
| 契約の新規追加で「保存」を押しても詳細ページに遷移しない | **不合格** | 不具合 4 として修正 |
| 商談の新規作成後に画面遷移しない | **不合格** | 同上 |
| 連絡先の新規作成後に画面遷移しない | **不合格** | 同上 |
| member で取引先を選択できない | **仕様どおり。ただし業務が回らない** | 不具合 5 として仕様変更 |

なお Claude 側の環境でリード新規作成の submit が発生しなかった件は、
同じ「遷移しない」事象の一部だったと考えられる（保存自体は成功していた可能性がある）。
修正 4 の適用後に改めて確認すること。
