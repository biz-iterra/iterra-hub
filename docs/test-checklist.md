...
## リリース: 2026-08-14 スクロール崩れの修正 + 全体監査（マイグレーション 4 本。**本番未反映**）
...
- 変更領域: `AppShell` の `<main>`、`src/lib/app-scroll.ts` / `src/hooks/useScrollLock.ts`（新設）、
  モーダル 7 つ、`src/actions` 8 ファイル、マイグレーション 4 本
  （`20260814100001`〜`100004`）
- 対応タスク: T-0089 / T-0090 / T-0091 / T-0092 / T-0093 / T-0094 / T-0095 / T-0096 /
  T-0097 / T-0098 / T-0099 / T-0100 / T-0101
...
| Gate 3-3 E2E スモーク (08 ランク S) | 2026-08-14 | Claude | 合格 | E2E 36 件。**初回は 3 件が落ちたが、いずれも `.next` を作り直した直後のコンパイル待ちによるタイムアウト**（詳細は下記）。温まった状態で再実行して全件緑 |
...
- **初回 E2E の 3 件（E2E-04 / E2E-07 / E2E-17）と総ざらいの失敗**: すべて
  「Test timeout of 90000ms exceeded」「waitForURL: Timeout」。切り分けのため
  `.next` を作り直した直後で、各ページの初回コンパイルが上限を超えていた。
  dev サーバーが温まった状態で再実行し 4 件とも緑。**コードの退行ではない**
...
- IT-DEALHIST-01〜04（`scripts/test-db/cases/13-deal-history-trigger.mjs`）: ディールの履歴トリガー
- CMN-13 / CMN-14 / CMN-15（07）、ADM-03b（07）、CNT-42（04）

### 申し送り

- **本番反映にはマイグレーション 4 本の適用が必要**（`20260814100001`〜`100004`）。
  `20260814100003` は RLS の変更を含むので、反映後に
  「ディールの活動が担当者に見える」「見られないディールの活動が見えない」を確認すること
- 残課題として T-0102（初期履歴が DB 関数側に残る）・T-0103（連携プロファイルの upsert）・
  T-0104（住所更新の 2 テーブル書き込み）を起票済み
