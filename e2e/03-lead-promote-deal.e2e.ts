...
test.describe("E2E-03", () => {
  ...
  test("リードを Opportunity へ昇格すると Company / Contact / Deal が生成される @smoke", async ({ page }) => {
    ...
    await fieldByLabel(page, "ステージ").selectOption({ label: "オポチュニティ" });
    await expect(page.getByText("このステージではディールが自動生成されます")).toBeVisible();

    await page.getByRole("button", { name: "保存" }).first().click();
    const promoteModalHeading = page.getByRole("heading", { name: "オポチュニティ に昇格します" });
    await expect(promoteModalHeading).toBeVisible();
    // 事業者種別=法人 なので事業者情報・連絡先・取引先・ディールが生成される旨の案内が出る。
    ...
    // **リードを先に消す。** リードが Opportunity のままディールを参照していると、
    // ステージ要件のトリガーがディールの削除を拒否する（docs/database-design.md §24.3）。
    // 業務では「リードのステージを下げてからディールを消す」が正規の手順だが、
    // ここは後片付けなのでリードごと消す
