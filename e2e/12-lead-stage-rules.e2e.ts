...
/**
 * E2E-12 [S] リードステージと実体（ディール・契約）の整合規則
 ...
 * 規則は「ディール・契約が無ければ先へ進めない」なので、他のシナリオと違い
 * 失敗系が主役になる。
 */
test.describe("E2E-12", () => {
  ...
  test("ステージ要件を満たさない遷移は画面から行えない @smoke", async ({ page }) => {
    ...
    // ---- 1. 新規作成ではディールが要るステージを選べない（LD-26）----
    await page.goto("/leads/new");
    const stageSelect = fieldByLabel(page, "ステージ *");
    await expect(stageSelect).toBeVisible();

    const stageLabels = await stageSelect.locator("option").allInnerTexts();
    // ディールが無いリードを Sales 以降で作れてしまうと、DB トリガーに弾かれるだけになる
    expect(stageLabels).not.toContain("ディール");
    expect(stageLabels).not.toContain("オポチュニティ");
    expect(stageLabels).not.toContain("取引先");
    // ディールが要らないステージは従来どおり選べる
    expect(stageLabels).toContain("リード獲得");
    expect(stageLabels).toContain("ナーチャリング");

    await fieldByLabel(page, "リード名 *").fill(leadName);
    await stageSelect.selectOption({ label: "ナーチャリング" });
    ...
    // 何が足りないかが読める文言で拒否される
    await expectErrorToastAndClose(page, "ディールが必要です");

    // ステージは変わっていない（拒否なので保存自体が起きていない）
    await page.goto(`/leads/${leadId}`);
    await expect(page.getByText("ナーチャリング").first()).toBeVisible();

    // ---- 3. Sales へ進めるとディールが作られ、ステータスも残る（LD-24）----
    await page.goto(`/leads/${leadId}/edit`);
    // **同じ保存でリード名も変える。** 昇格は DB の値を読むため、
    // 「今回入力した値」が届かないとディールに古い名前が付く。
    ...
    await fieldByLabel(page, "ステージ *").selectOption({ label: "ディール" });
    ...
    await expect(
      page.getByRole("heading", { name: "ディール に昇格します" })
    ).toBeVisible();
    await page.getByRole("button", { name: "昇格する" }).click();
    await expectSuccessToast(page, "ディールに昇格しました");

    await page.waitForURL(/\/leads\/[0-9a-f-]{36}$/);
    // ディールが作られ、リードから辿れる
    const promotedLink = page.getByRole("link", { name: "ディール昇格済み" });
    ...
    // ---- 後片付け（リード → ディール → 事業者情報・連絡先の順。理由は §24.3）----
    await page.goto(`/leads/${leadId}/edit`);
    ...
    await page.getByRole("button", { name: "削除", exact: true }).click();
    await page.getByRole("button", { name: "削除する" }).click();
    await expectSuccessToast(page, "ディールを削除しました");
  });
});
