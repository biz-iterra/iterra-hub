 ...
 * 法人格の綴り（展開後の正式名称）。
 *
 * **DB 関数 `company_sort_key` の除去リストと対で持つ。**
 * マスタ（`corporate_types`）は運用中に増えるが、こちらは読みと並びを
 * 決めるためのものなので固定でよい。増やすときは両方を直すこと。
 */
const CORPORATE_FORMS = [
  ...
  "特定非営利活動法人",
  "NPO法人",
  // 実データに全角表記がある。表記のまま落とせるよう別項目で持つ
  "ＮＰＯ法人",
  "地方独立行政法人",
  "独立行政法人",
  "国立研究開発法人",
 ...
 * 法人格を取り除いた名称を返す。
 *
 * フリガナは事業者の呼び名として使うので「カブシキガイシャ〜」は要らない。
 * 前株・後株のどちらでも同じ結果になる。
 * 法人格を落とした後に頭へ残る括弧なども取り除く。
 */
export function stripCorporateType(name: string | null | undefined): string {
  const expanded = formatCompanyName(name);
  if (!expanded) return "";

  // 長い綴りから当てる。「独立行政法人」が先に当たると
  // 「地方独立行政法人」の「地方」だけが残ってしまう
  const pattern = [...CORPORATE_FORMS]
