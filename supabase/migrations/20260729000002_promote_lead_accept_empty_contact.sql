-- ============================================================
-- promote_lead_to_deal: 連絡先の空文字を NULL として扱う
--
-- 背景:
--   supabase gen types は関数引数の NULL 許容を表現できず、
--   TEXT 引数は常に `p_contact_email: string`（非 null）として生成される。
--   そのため呼び出し側で `string | null` を渡すと型エラーになる。
--
--   キャストで回避すると Database 型を適用した意味が薄れるため、
--   関数側で「空文字 = 未入力」とみなす仕様に変更する。
--   呼び出し側は null ではなく '' を渡す。
--
-- 変更点は連絡先 2 引数の判定のみ。他のロジックは 20260728000001 と同一。
-- ============================================================
...
CREATE OR REPLACE FUNCTION promote_lead_to_deal(
  ...
  p_contact       JSONB,
  p_contact_email TEXT,    -- '' = 未入力
  p_contact_phone TEXT,    -- '' = 未入力
  p_account       JSONB,
  p_deal          JSONB
) RETURNS JSONB
LANGUAGE plpgsql
...
DECLARE
  ...
  v_stage_id   UUID;
  v_status_id  UUID;
  -- 空文字は未入力として扱う（生成型が NULL 許容を表現できないため）
  v_email      TEXT := NULLIF(p_contact_email, '');
  v_phone      TEXT := NULLIF(p_contact_phone, '');
BEGIN
  ...
  IF v_email IS NOT NULL THEN
    ...
    VALUES (v_contact_id, v_email, 'work', TRUE, v_user_id, v_user_id);
  END IF;

  IF v_phone IS NOT NULL THEN
    INSERT INTO contact_phones (contact_id, phone, label, is_primary, created_by, last_updated_by)
    VALUES (v_contact_id, v_phone, 'work', TRUE, v_user_id, v_user_id);
  END IF;
  ...
  -- deal_status_histories.stage_id は NOT NULL
...
COMMENT ON FUNCTION promote_lead_to_deal IS
  'Lead → Deal 昇格を単一トランザクションで実行する。連絡先の空文字は未入力として扱う。値の整形は呼び出し側（Server Action）の責務。';
