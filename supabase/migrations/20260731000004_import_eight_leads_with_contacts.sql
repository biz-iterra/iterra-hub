-- ============================================================
-- Eight 名刺 CSV の取込に「法人・連絡先の作成」を追加する
--
-- 変更点（20260730000004 からの差分）:
--   - Lead を書いたあと resolve_or_create_company / resolve_or_create_contact を
--     呼び、leads.company_id / leads.contact_id を埋める
--   - 取引先（Account）は作らない。契約後に作る運用に変えたため
--
-- 名刺は「リード」であると同時に「連絡先」でもある、という整理に合わせる。
-- 解決ロジック自体は 20260731000003 の関数側にあり、既存リードの遡及作成と共有する。
-- ============================================================
...
COMMENT ON FUNCTION import_eight_leads(JSONB, JSONB, JSONB, JSONB) IS
  'Eight 名刺 CSV の取込。addresses/leads/companies/contacts/lead_activities/lead_import_records を単一トランザクションで書く。取引先は契約後に作るため触らない';
