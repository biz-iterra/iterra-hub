...
-- 取引先コードは「見せるだけ」にする
--
-- 背景（2026-08-05 の不具合報告）:
--   差分画面で取引先コードに「CRM → freee」を選ぶと、freee が 400 を返していた。
--     不正なリクエストです。 / このAPIでは code の指定はできません。
--   freee の取引先コードは**新規登録（POST）でしか指定できず、更新（PUT）では
--   受け付けない**（公式 SDK でも PartnerCreateParams にだけ code がある）。
--
--   逆向きも通らない。`companies.company_code` は generate_company_code() が
--   採番する VARCHAR(10) UNIQUE NOT NULL で、freee の値では上書きできない。
--   それにもかかわらず apply_freee_values_to_crm() には code の分岐が無く、
--   選んでも**何も起きないまま成功として記録されていた**。
--
--   そこで担当者名・メールと同じく「反映できない項目」として明示的に弾く。
--   差分画面では値だけ見せ、揃えるときは freee の画面で人が入力する（§26.8）。
-- ============================================================
...
'freee → CRM の取り込み。担当者名・メール・事業者コードは受け付けない（CRM が正本）';
