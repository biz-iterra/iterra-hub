-- ============================================================
-- 契約を商談に紐づかない状態で持てるようにする（T-0065）
--
--   2026-08-07 に商談の編集画面へ「既存の契約を紐づける」を置いたが、
--   `contracts.deal_id` が NOT NULL のため、契約は必ずどれかの商談に属していた。
--   つまり「紐づける」が**必ず他の商談から奪う付け替え**になっていた。
--
--   利用者判断で `deal_id` を任意にし、
--   **紐づけ候補を「どの商談にも紐づいていない契約」だけ**にする。
--   商談から外す（= NULL に戻す）操作も入る（T-0067）。
--
--   1 商談に複数の契約が下がる作りは変えない。
-- ============================================================
...
'ディール（任意）。どの商談にも紐づかない契約を持てる。商談の編集画面から紐づけ／解除する';
...
--
--   判定対象の商談は **OLD.deal_id**（外す前に付いていた商談）。
...
'契約の論理削除・商談からの紐づけ解除で「ステージは取引先なのに契約が無い」状態を作らせない';
